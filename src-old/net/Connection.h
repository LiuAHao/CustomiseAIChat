...
private:
    ...
    Timestamp lastatime_;       //创建Connection时为当前时间戳，每接收一个报文，更新为当前时间戳
...
public:
    ...
    void send(const char* data,size_t size);        //不管是什么线程，调用该函数发送数据
    void sendinloop(std::string message);      //为IO线程时调用此函数；为工作线程时将此函数传给IO线程

    bool timeout(time_t now,int val);       //判断Connection是否连接太久
};